...
- `src/discord/command-registry.ts` is the authoritative source for every Slash Command in this application. Deploy uses Discord bulk overwrite to replace the complete command set in the selected global or guild scope; commands registered manually or by another system in that same scope are intentionally removed on the next deploy.
