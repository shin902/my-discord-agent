...
const runner = createCronRunner({
  appendInbox,
  getDefaultDiscordClient: () => client,
  getDiscordClientForGroupName: async () => client,
});
...
describe("cron runner integration", () => {
  it.each([
    ...
    "jobs/../../evil.ts",
    "/etc/passwd.ts",
  ])("rejects unsafe handler %s", async (path) => {
    await expect(runner.loadHandlerFn(path)).rejects.toBeInstanceOf(
      NonRetryableError,
    );
  });
  ...
  it("uses destination as the session for direct destination jobs", async () => {
    ...
    expect(appendInbox).toHaveBeenCalledWith(
      expect.objectContaining({ sessionId: "thread" }),
    );
  });
