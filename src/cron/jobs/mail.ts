...
type GraphPayload =
  | z.infer<typeof EmailPageSchema>
  | z.infer<typeof BodySchema>;
const GraphResponseSchema = z.object({
  ok: z.boolean(),
  status: z.number().optional(),
  json: z.custom<() => Promise<GraphPayload>>(),
  text: z.custom<() => Promise<string>>(),
});
...
export type MailDependencies = {
  ...
  runTextOnlyAgent: typeof runTextOnlyAgent;
  appendMessage: typeof appendMessage;
};
const defaults: MailDependencies = {
  fetch,
  getProxyPort,
  findGroupByName,
  resolveModelConfig,
...
export function createMailHandler(dependencies: MailDependencies = defaults) {
  const graphUrl = (path: string) =>
    `http://localhost:${dependencies.getProxyPort()}/graph${path}`;
  ...
  const listUnreadEmails = async (): Promise<UnreadEmail[]> => {
    const data = EmailPageSchema.parse(
      await graphFetch(
        `/me/mailFolders/inbox/messages?$top=${UNREAD_FETCH_LIMIT}&$select=id,subject,from&$orderby=receivedDateTime asc&$filter=isRead eq false`,
      ),
    );
  ...
  const generateSummary = async (emailText: string, ctx: CronContext) => {
    const groupConfig = ctx.groupName
      ? await dependencies.findGroupByName(ctx.groupName)
      : undefined;
    const { provider, modelId } = await dependencies.resolveModelConfig(
      groupConfig?.model,
    );
    const model = await dependencies.resolveModel(provider, modelId);
    return dependencies.runTextOnlyAgent({
      systemPrompt: ctx.prompt ?? DEFAULT_SUMMARY_PROMPT,
      model: {
        ...model,
        baseUrl: `http://localhost:${dependencies.getProxyPort()}/${provider}`,
      },
  ...
  return async function handler(ctx: CronContext): Promise<void> {
    if (!ctx.channelId || !ctx.groupName) {
      console.error("[mail] channelId/groupName が設定されていません");
      return;
    }
    const groupConfig: GroupConfig | undefined =
      await dependencies.findGroupByName(ctx.groupName);
    const { provider, modelId } = await dependencies.resolveModelConfig(
      groupConfig?.model,
    );
    await dependencies.resolveModel(provider, modelId);
    ...
    for (const meta of await listUnreadEmails()) {
      try {
        ...
        const result = await generateSummary(emailText, ctx);
        ...
        const sent = await channel.send(chunks[0] ?? "(要約なし)");
