...
function deps(
  fetchImpl: MailDependencies["fetch"],
  agentMessage: AgentMessage,
): MailDependencies {
  return {
    fetch: fetchImpl,
    getProxyPort: () => 1234,
    findGroupByName: vi.fn(async () => undefined),
    resolveModelConfig: vi.fn(async () => ({
      provider: "test",
      modelId: "model",
    })),
    resolveModel: vi.fn(async () => model),
    runTextOnlyAgent: vi.fn(async () => ({ text: "要約", agentMessage })),
...
describe("mail cron delivery boundary", () => {
  ...
  it("不完全なassistant応答は送信・既読化しない", async () => {
    const fetchMock = vi.fn<typeof fetch>();
    for (const response of responses().slice(0, 2))
      fetchMock.mockResolvedValueOnce(response);
    const send = vi.fn();
    await createMailHandler(deps(fetchMock, assistant("error")))(
      context({ type: 0, send }),
    );
