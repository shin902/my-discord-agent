...
export function createIssueTriageHandler(
  dependencies: IssueTriageDependencies = {
    ...
    writeFile: async (_path, data) => {
      await writeFile(STATE_PATH, data, "utf-8");
    },
    getProxyPort,
    fetch: async (input, init) => {
      ...
      return {
        ...
        json: async () =>
          z.array(GitHubIssueSchema).parse(await response.json()),
        text: () => response.text(),
      };
    },
  },
  ...
  const fetchIssuesByCreator = async (
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `GitHub API エラー ${res.status ?? 0}: ${text.slice(0, 200)}`,
        );
      }
  ...
  return async function handler(ctx: CronContext): Promise<void> {
    ...
    for (const issue of targets) {
      try {
        ...
        console.error(
          `[issue-triage] Issue #${issue.number} の投入に失敗:`,
          err,
        );
      }
    }
