...
const AGENT_EXECUTION_JOB_PREDICATE = `${QUEUE_JOB_PREDICATE} AND json_type(payload_json,'$.jobKind') IS NULL`;
...
function counts(
  ...
  for (const row of db
    .prepare(
      `SELECT status,COUNT(*) AS count FROM ${table} WHERE ${predicate} GROUP BY status`,
    )
