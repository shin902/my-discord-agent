...
describe("runtime retention", () => {
  it("keeps active and ambiguous rows in dry-run and prune", async () => {
    ...
    try {
      ...
      const terminal = repo.enqueue(
        { ...payload, content: "terminal" },
      ...
      repo.db
        ...
        .run("2020-01-01T00:00:00.000Z", terminal.id);
      repo.db
        .prepare(
          "INSERT INTO deliveries(id,job_id,status,payload_json,created_at,updated_at) VALUES(?,?,?,?,?,?)",
        )
        .run(
          "ambiguous-1",
          active.id,
  ...
  it("does not delete when archive creation fails", async () => {
    const repo = new QueueRepository(openRuntimeDb(":memory:"));
    const dir = join(
      await mkdtemp(join(tmpdir(), "retention-fail-")),
      "not-a-dir",
    );
    ...
    try {
      ...
      await expect(
        pruneRetention(
          repo.db,
          { archiveDir: dir, jobs: { completed: 1 } },
          { at: new Date("2025-01-01T00:00:00.000Z") },
        ),
      ).rejects.toThrow();
  ...
  it("requires an archive directory only for non-dry pruning and never overwrites exports", async () => {
    ...
    try {
      ...
      repo.db
        ...
        .run("2020-01-01T00:00:00.000Z", job.id);
      const dry = await pruneRetention(
        repo.db,
        { jobs: { completed: 1 } },
        { at, dryRun: true },
      );
      ...
      await expect(
        pruneRetention(repo.db, { jobs: { completed: 1 } }, { at }),
      ).rejects.toThrow("archiveDir is required");
      expect(repo.get(job.id)).toBeDefined();
      const first = await pruneRetention(
        repo.db,
        { archiveDir: dir, jobs: { completed: 1 } },
        { at },
      );
      ...
      try {
        const secondJob = secondRepo.enqueue({
          ...payload,
          content: "second",
        }).job;
        secondRepo.db
          ...
          .run("2020-01-01T00:00:00.000Z", secondJob.id);
        const second = await pruneRetention(
          secondRepo.db,
          { archiveDir: dir, jobs: { completed: 1 } },
          { at },
        );
  ...
  it("does not delete earlier batches when a later archive export fails", async () => {
    ...
    try {
      ...
      try {
        ...
        repo.db
          .prepare(
            "UPDATE jobs SET status='completed',result_state='succeeded',succeeded=1,updated_at=? WHERE id IN (?,?)",
          )
          .run("2020-01-01T00:00:00.000Z", firstJob.id, secondJob.id);
        const stamp = at.toISOString().replace(/[:.]/g, "-");
        const blockedSecondTarget = join(
          dir,
          `runtime-retention-${stamp}-1-fixed-retention-test-id.jsonl`,
        );
  ...
  it("archives and deletes eligible deliveries before their terminal parent", async () => {
    ...
    try {
      ...
      expect(
        repo.db
          .prepare("SELECT 1 FROM deliveries WHERE id=?")
          .get("old-failed"),
      ).toBeUndefined();
      const archive = (
        await Promise.all(
          result.archivePaths.map((archivePath) =>
