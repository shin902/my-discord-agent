...
import {
  createGraphAuth,
  type GraphAuthClient,
  type GraphAuthFileSystem,
} from "./graph-auth.js";
...
const account = (id: string): AccountInfo => ({
  homeAccountId: id,
  environment: "env",
  tenantId: "tenant",
...
function setup(
  ...
  overrides: Partial<{
    silent: GraphAuthClient["acquireTokenSilent"];
    device: GraphAuthClient["acquireTokenByDeviceCode"];
  }> = {},
) {
  const cache = {
  ...
  const client: GraphAuthClient = {
    getTokenCache: () => cache,
    acquireTokenSilent:
      overrides.silent ?? vi.fn().mockRejectedValue(new Error("expired")),
    acquireTokenByDeviceCode:
      overrides.device ?? vi.fn().mockResolvedValue(null),
  };
  const fs: GraphAuthFileSystem = {
  ...
  return {
    ...
    cache,
    fs,
    auth: createGraphAuth({ fileSystem: fs, createClient: () => client }),
  };
}
...
describe("getGraphAccessToken", () => {
  it("requires initialization", async () => {
    const { auth } = setup();
    await expect(auth.getGraphAccessToken("graph")).rejects.toThrow(
  ...
  it("returns and persists a silently acquired token", async () => {
    ...
    await expect(state.auth.getGraphAccessToken("graph")).resolves.toBe(
      "silent",
    );
    expect(silent).toHaveBeenCalledWith(
      expect.objectContaining({
        account: expect.objectContaining({ homeAccountId: "one" }),
  ...
  it("falls back to device code and removes old accounts", async () => {
    ...
    expect(state.cache.removeAccount).toHaveBeenCalledWith(
      expect.objectContaining({ homeAccountId: "one" }),
    );
  });
  ...
  it("rejects when device flow returns no result", async () => {
    const state = setup();
    await state.auth.initGraphAuth("graph", config);
    await expect(state.auth.getGraphAccessToken("graph")).rejects.toThrow(
