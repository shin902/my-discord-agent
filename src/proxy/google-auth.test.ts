...
): GoogleAuthFileSystem => ({
  readFile: vi
    .fn()
    .mockImplementation(
      async () => cache ?? Promise.reject(new Error("ENOENT")),
    ),
  writeFile: writer,
...
describe("getGoogleAccessToken", () => {
  it("requires initialization", async () => {
    const { getGoogleAccessToken } = createGoogleAuth({
      fileSystem: fileSystem(),
    });
    await expect(getGoogleAccessToken("google")).rejects.toThrow(
  ...
  it("refreshes expired tokens and persists them", async () => {
    ...
    const fetcher = vi
      .fn()
      .mockResolvedValue(response({ access_token: "new", expires_in: 3600 }));
    const auth = createGoogleAuth({
      fetch: fetcher,
      fileSystem: fileSystem(
        JSON.stringify({ refreshToken: "refresh", expiresAt: 0 }),
        writer,
      ),
    });
    await auth.initGoogleAuth("google", config, "secret");
  ...
  it("starts device flow and persists its result", async () => {
    ...
    const auth = createGoogleAuth({
      fetch: fetcher,
      fileSystem: fileSystem(undefined, writer),
    });
    await auth.initGoogleAuth("google", config, "secret");
    await expect(auth.getGoogleAccessToken("google")).rejects.toBeInstanceOf(
      GoogleAuthRequiredError,
    );
  ...
  it("returns the same pending device flow", async () => {
    ...
    expect(
      fetcher.mock.calls.filter(
        (call) =>
          String(call[0]) === "https://oauth2.googleapis.com/device/code",
      ),
    ).toHaveLength(1);
    vi.useRealTimers();
  });
