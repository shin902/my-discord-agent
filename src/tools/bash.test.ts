...
function getText(
  result: Awaited<ReturnType<ReturnType<typeof createBashTool>["execute"]>>,
): string {
...
describe("bashTool", () => {
  it("コマンドの stdout を返す", async () => {
    const exec: ExecFn = vi
      .fn()
      .mockResolvedValue({ stdout: "hello\n", stderr: "" });
    const result = await createBashTool(exec).execute(
      "id",
      { command: "echo hello" },
      undefined,
      undefined,
  ...
  it("stderr もキャプチャして返す", async () => {
    const exec: ExecFn = vi
      .fn()
      .mockResolvedValue({ stdout: "", stderr: "warn\n" });
    const result = await createBashTool(exec).execute(
      "id",
      { command: "echo warn >&2" },
      undefined,
  ...
  it("コマンド失敗時はエラーをスローする", async () => {
    const exec: ExecFn = vi
      .fn()
      .mockRejectedValue(
        Object.assign(new Error("failed"), { stderr: "error output\n" }),
      );
    await expect(
      createBashTool(exec).execute(
        "id",
        { command: "exit 1" },
        undefined,
        undefined,
