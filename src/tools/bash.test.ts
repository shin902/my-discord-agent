...
function mockFailure(err: Error) {
  mockExec.mockImplementation((_cmd, _opts, cb) => {
    cb?.(err, "", (err as NodeJS.ErrnoException & { stderr?: string }).stderr ?? "");
    return {} as ChildProcess;
  });
}
...
describe("bashTool", () => {
  it("コマンドの stdout を返す", async () => {
    mockSuccess("hello\n");
    const result = await bashTool.execute("id", { command: "echo hello" }, undefined, undefined);
    expect(getText(result)).toBe("hello");
  });

  it("stderr もキャプチャして返す", async () => {
    mockSuccess("", "warn\n");
    const result = await bashTool.execute("id", { command: "echo warn >&2" }, undefined, undefined);
    expect(getText(result)).toContain("warn");
  });

  it("コマンド失敗時はエラーをスローする", async () => {
    const err = Object.assign(new Error("failed"), { stderr: "error output\n" });
    mockFailure(err);
    await expect(
  ...
  it("出力なしのコマンドは (出力なし) を返す", async () => {
    mockSuccess("", "");
    const result = await bashTool.execute("id", { command: "true" }, undefined, undefined);
