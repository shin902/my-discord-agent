...
const nativeFileSystem: SessionFileSystem = {
  ...
  append: (file, data) =>
    appendFile(file, data, { encoding: "utf-8", mode: 0o666 }),
  makeDirectory: (dir) =>
    mkdir(dir, { recursive: true, mode: 0o777 }).then(() => undefined),
...
function createSessionService(directory: string, fs: SessionFileSystem) {
  const filePath = (group: string, session: string) =>
    path.join(directory, group, `${session}.jsonl`);
  ...
  const append = async (
    ...
    const sanitized =
      rest.role === "assistant"
        ? {
            ...rest,
            content: rest.content.filter((block) => block.type !== "thinking"),
          }
        : rest;
