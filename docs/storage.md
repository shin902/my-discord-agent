...
`data/screen-captures.sqlite`は画像本体と処理状態を一緒に保存するhost専用DBです。`completed_at IS NULL`を未完了の正本とし、AgentがActivity Memoryを更新した後に完了時刻を保存します。WAL運用のため稼働中のmain file単独copyは避け、SQLite backup APIを使用してください。runtime DB backupには含まれません。設定・schema・Mac送信・retentionは [画面画像の収集と要約](screen-capture.md) を参照してください。
