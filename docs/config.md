...
| `xSavedGallery` | — | `enabled`（既定: false）、`port`（既定: 8789、1–65535）。有効時は `origin`（HTTPS `.ts.net` origin、末尾 `/` なし）と `allowedLogin`（自分のTailscale login）が必須。receiverとは別のlocalhost listener。[Gallery設定・認証・編集](x-saved.md#gallery-browse-and-edit-over-tailscale)を参照。変更後は再起動が必要 |
