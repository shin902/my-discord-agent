...
`seen_liked` and `seen_bookmarked` are sticky history flags. If a like or bookmark later disappears from X, the local record remains searchable. Agent-managed status, tags, summaries, and notes are stored only in `x-saved.sqlite` and are never overwritten by BirdClaw ingest.
...
The setup command asks BirdClaw to import only `likes,bookmarks,profiles`, ingests the resulting local data into `x-saved.sqlite`, marks that first bulk import as the historical baseline, and writes a SQLite backup.

Historical baseline items remain searchable but do not appear in normal `pending` triage. This prevents a multi-thousand-item archive import from flooding the daily agent queue. To intentionally triage the whole backlog:

```bash
pnpm x-saved:setup -- --keep-backlog ~/Downloads/twitter-archive.zip
```
...
The baseline marker is initialized only once, so rerunning setup does not silently reclassify later live items.
...
For multiple BirdClaw accounts, set `settings.account` to the stored account ID or handle. The same selector is used for BirdClaw live sync and local ingest.
...
python3 SKILLS/x-saved/scripts/x-saved.py mark <tweet-id> try --priority 90
```
...
- `x_items` — post body, author, URL, sticky like/bookmark history, baseline flag
- `x_item_state` — `inbox`, `reviewed`, `keep`, `try`, `done`, `ignore`, plus priority/summary/note
- `x_tags` — many-to-many local labels
- `x_sync_runs` — sync/ingest health and audit data
- `x_meta` — one-time local metadata such as baseline initialization
