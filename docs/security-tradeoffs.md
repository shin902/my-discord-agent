...
**残存リスク:** 互換用`agent-reach.sh`（bash経由のcurl/yt-dlp等）でのDNSリバインディング攻撃と組み合わさった場合、悪意あるURLを経由してLAN内部サービスへ到達できる可能性がある。ただし実行環境がサンドボックスコンテナ内であるため影響範囲は限定的。

**対策済み内容:**
- `agent-reach.ts` の`isPrivateAddress()`で解決済みIPをチェック（全アドレス検査）
- `allowLoopback()` は不要なため削除済み（コンテナ自身への接続を排除）

**注意:** 互換用シェルスクリプトはプロトコルチェック（http/https）のみで、DNS解決済みIPのチェックを行わない。
...
**場所:** `src/tools/agent-reach.ts` — `lookup()`後に`curl`/`yt-dlp`が内部で再度DNS解決する

**内容:** Node.jsの`lookup()`でIPを検証した後、実際のコマンド実行までの間にDNSレコードが書き換わる可能性がある。互換用シェルスクリプトでは事前のIP検証を行わない。
