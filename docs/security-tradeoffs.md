...
**残存リスク:** `agent-reach` ツールでのDNSリバインディング攻撃と組み合わさった場合、悪意あるURLを経由してLAN内部サービスへ到達できる可能性がある。ただし実行環境がサンドボックスコンテナ内であるため影響範囲は限定的。
...
**理由:** エージェントの主要機能（Web検索・URL取得）にはインターネットアクセスが必須。`agent-reach` ツールのドメインは大体固定されているためホワイトリストは技術的には可能だが、エージェントとしての汎用性を損なうため現時点では採用しない。
