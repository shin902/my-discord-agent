...
2. 新規プロジェクトを作成（または既存プロジェクトを選択）（Google Cloudの右側のプロジェクト名から選択、新規作成どっちもできる）
...
2. 名前を決める・ユーザーサポートメールはメールアドレスをそのまま入力・次へ を押す・
3. User Type は **「外部」** を選択（個人 Google アカウントの場合）
4. 連絡先情報の入力・ポリシーに同意する・一旦終了する
5. 「データアクセス」「スコープ」の追加で `https://www.googleapis.com/auth/calendar` を追加（手動追加する形）
