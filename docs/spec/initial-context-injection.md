...
1. `system-prompt-snapshot` の内容、または AGENTS.md 不在時の `DEFAULT_SYSTEM_PROMPT`
...
- **グループsystem promptが空文字**: `system-prompt-snapshot` を空内容で保存し、DEFAULT_SYSTEM_PROMPT も除外する。結果として systemPrompt は固定time anchor（+ skills / request append）のみになる。
