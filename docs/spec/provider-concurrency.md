...
通常の Discord メッセージとcronは、共通AgentConfigの `group → channel → cron job` 解決後の `model` を使い、最後に `defaultModel` で補完する。解決した `provider` と同名の `providers.json` エントリから `concurrency` を取得して LLM ロックを取る。
