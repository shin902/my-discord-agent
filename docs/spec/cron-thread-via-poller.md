...
`sessionMode: "destination"` では、宣言的 `item-thread` の配送workerが作成したスレッドIDがそのままセッションIDになる。ユーザーがそのスレッドに返信すると、同じセッションIDで会話履歴が引き継がれる。cronの各投入は新しいjob/threadを作るため、別のcron実行からセッションを再開しない。
...
| ファイル | 変更内容 |
|---|---|
| `queue/types.ts` / `queue/repository.ts` | cronの投稿方法・セッション戦略・job payloadとclaim/provisioningを保持 |
