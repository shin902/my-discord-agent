...
応答中にtrim後が完全一致する独立行 `<NO_REPLY>` があれば、cron・通常会話を問わず正常完了してDiscord deliveryを作らない。inlineの言及は通常どおり配送する。cronの`noReply: true`はこのプロトコルをsystem promptで案内するだけで、判定自体は常時有効である。全delivery modeで設定でき、Mail/RSS sourceは無配信でも正常にACK/finalizeする。`new-thread` + `destination` はthread IDをAIセッションに使うため実行前にスレッドを作成し、NO_REPLY時も投稿のないスレッドが残る。
