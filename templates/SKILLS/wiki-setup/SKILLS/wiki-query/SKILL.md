...
- Append a log entry.
  ...
  `log.md` is append-only and may be too large to read in full. Before appending, use `bash` with `tail -n 20` (or `tail -n 10`) to inspect only its last lines. Append with `printf ... >> log.md` or `cat >> log.md`. Never use full-file `edit`/`write` or read-and-write-back scripts.
