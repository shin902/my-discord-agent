---
name: "wiki-search"
description: "インデックスファイルだけでは不十分な場合に、LLMが管理するwikiのページ全体を検索する。ingest時（エンティティを新規作成するか既存ページを編集するか判断する前に、すでにそのエンティティに言及しているページを探すため）、query時（関連ページを特定するため）、lint時に使用する。依存ライブラリ不要のローカル検索を提供する。wikiが数百ページを超えたらwiki-search-ftsへの移行をユーザーに提案すること。"
---
