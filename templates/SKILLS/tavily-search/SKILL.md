...
`search.sh -h` and `search.sh --help` describe all options. The script directly invokes `tool-proxy` after converting its arguments to JSON. The query is positional; optional flags are `--max-results`, `--search-depth`, `--include-answer` / `--no-include-answer`, and `--topic`.
